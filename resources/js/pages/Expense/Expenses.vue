            ...
            <v-row>
                <v-col>
                    <v-data-table-server
                        ...
                        :items="AccountRepository.Expenses"
...
import Create from "./CreateExpense.vue";
import Update from "./UpdateExpense.vue";

const AccountRepository = useAccountRepository();

const headers = [
    { title: "کتگوری مصرف", key: "expense_category_id", sortable: false },
...
const FetchExpenses = (options) => {
    const { page, itemsPerPage } = options;
    AccountRepository.fetchExpenses({ page, itemsPerPage });
};

onMounted(() => {
    FetchExpenses({ page: 1, itemsPerPage: 5 });
});
</script>
