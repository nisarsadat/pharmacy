            ...
            <v-row>
                <v-col>
                    <v-data-table-server
                        ...
                        :items="AccountRepository.ExpenseCategories"
...
import Create from "./CreateExpenseCategory.vue";
import Update from "./UpdateExpenseCategory.vue";
...
const headers = [
    ...
    { title: "نوت", key: "details", sortable: false },
...
const deleteItem = (id) => {
    AccountRepository.deleteExpenseCategory(id);
};
...
const editItem = (id) => {
    ...
    if (
        !AccountRepository.ExpenseCategory ||
        Object.keys(AccountRepository.ExpenseCategory).length === 0
    ) {
        AccountRepository.fetchExpenseCategory(id)
            .then(() => {
...
const fetchExpenseCategories = (options) => {
    const { page, itemsPerPage } = options;
    AccountRepository.fetchExpenseCategories({ page, itemsPerPage });
};

onMounted(() => {
    fetchExpenseCategories({ page: 1, itemsPerPage: 5 });
});
</script>
